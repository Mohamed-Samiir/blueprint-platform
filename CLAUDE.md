  ...
  foundation/                  — ✅ the only pillar with real generator logic
    ...
    src/
      ...
      generators/preset/
        ...
        schema.d.ts            — TS type for options (currently stale — see status)
        preset.spec.ts
        ...
        files/                 — templates COPIED verbatim (never compiled)
          ...
          shared/ui/button/…, shared/ui/utils/…    (real .ts spartan-derived source)
...
- `preset/files/**` = payload copied into the generated project → excluded from `tsconfig.lib.json`, shipped via the `assets` globs in `foundation/package.json` (`**/!(*.ts)` from `src`, plus an explicit `**/*.ts` glob rooted at `files/shared/ui`, plus `schema.json`, plus `compatibility.json`).
...
- **Copy-based generation, not npm-dependency-based, for anything a developer should be able to freely modify.** `@angular/cdk`, `@spartan-ng/brain`, `class-variance-authority`, `clsx`, `tailwind-merge` are real pinned npm dependencies (never copied, never modified). Everything under `shared/ui/`, `shared/components/`, `features/`, `layout/` is copied source the developer owns outright.
