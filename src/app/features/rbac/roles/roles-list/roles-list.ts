...
export class RolesList {
  ...
  protected readonly pendingAction = signal<{
    role: Role;
    kind: DeleteOrDeactivate;
  } | null>(null);
  private readonly actionDialog =
    viewChild.required<HlmAlertDialog>('actionDialog');

  protected readonly viewingUsersRole = signal<Role | null>(null);
  private readonly usersDialog =
    viewChild.required<HlmAlertDialog>('usersDialog');
  ...
  protected confirmPendingAction(): void {
    ...
    if (pending.kind === 'delete')
      this.rolesService.deleteRole(pending.role.id);
