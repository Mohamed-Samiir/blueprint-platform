...
export class PermissionForm {
      ...
      .map(({ module, permissions }) => ({
        ...
        rows: permissions
          ...
          .map((p) => ({
            ...
            disabledByCycle:
              !this.dependsOn().has(p.id) &&
              wouldCreateCycle(this.currentId, p.id, all),
          })),
      }))
  ...
  protected submit(): void {
    ...
    const { moduleId, newModuleName, key, label, isModuleRoot } =
      this.form.getRawValue();
    const resolvedModuleId =
      moduleId === NEW_MODULE
        ? this.permissionsService.addModule(newModuleName || 'Untitled module')
            .id
        : moduleId;
