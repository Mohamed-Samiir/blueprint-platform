...
@Component({
  ...
  providers: [
    provideIcons({ lucidePencil, lucidePlus, lucideSearch, lucideTrash2 }),
  ],
...
export class PermissionsList {
  ...
  protected isExpanded(moduleId: string): boolean {
    return (
      this.query().trim().length > 0 || this.expandedModuleIds().has(moduleId)
    );
  }
  ...
  protected readonly pendingDependents = computed(() => {
    ...
    return target
      ? resolveDependents(target.id, this.permissionsService.permissions())
      : [];
  });

  private readonly deleteDialog =
    viewChild.required<HlmAlertDialog>('deleteDialog');
